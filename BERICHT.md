...
## 7. Die drei Katas, 2026-09-22

Von GitLab (`gitlab.com/sharpsharp/{stringcalculator,gildedrose,tripservice}`) nach GitHub übernommen, weil das Training sonst zwei Seiten bräuchte. Die GitLab-Stände bleiben unverändert liegen.

Befund der GitLab-Repos: keine `pom.xml` (die IntelliJ-Dateien zeigten auf eine), Bibliotheken als Jars im Repo, Java-Level 1.8, beim StringCalculator der einzige Test unter `src/main`, keine README. Ohne Build-Werkzeug kann opencode kein `mvn test` als Gate nutzen.

| Repo | Inhalt | opencode |
|---|---|---|
...
| [gildedrose](https://github.com/sharpsharp-ai/gildedrose) | Legacy-Code ohne Tests (nur der `foo`-Starttest), ApprovalTests und JaCoCo bereit, `scripts/unabgedeckt.sh`, `scripts/approve.sh` | Rolle `test-autor` (nur `src/test`), zwei Commands: `/approval-test` (Golden Master, brute force über ein Raster) und `/characterization-test` (je Verzweigung ein benannter Test), je ein Skill |
...
Verifiziert: `mvn -q verify` in jedem Repo lokal und im frischen Clone von GitHub, Exit 0; CI aller drei Repos grün; je eine Probe mit `gpt-5.4-mini` (`protokolle/katas/README.md`): `/characterization-test` (damals `/charakterisiere`) erreichte in einem Lauf 36 von 36 Zweigen mit 14 Tests ohne Änderung am Produktivcode, `/approval-test` legte in einem Lauf ein Golden Master mit 350 genehmigten Zeilen an (ebenfalls 36 von 36 Zweigen, Produktivcode unverändert), `/coach` machte in drei Runden je genau einen Schritt (Frage, roter Test, Naht mit grünem verify) und hielt `TripService_Original` unangetastet.

Clean-Code-Report 2.0 (2026-09-22, in allen drei Kata-Repos als opencode-Skill `.opencode/skills/clean-code-report/`): `java .opencode/skills/clean-code-report/CleanCodeReport.java` oder in opencode `/clean-code-report`. Ein Java-Programm ohne Abhängigkeiten, das den Code mit dem Parser des JDK liest (kein Textmuster-Raten mehr) und 30 Code Smells in sechs Familien prüft, nach dem Katalog von Fowler in der Einteilung von Mäntylä und refactoring.guru: Bloaters (Long Method, Large Class, Long Parameter List, Too Many Temps, Primitive Obsession, Data Clumps), Object-Orientation Abusers (Switch Statements, Temporary Field, Public Mutable Field, Reference Comparison), Dispensables (Duplicate Code, Dead Code, drei Kommentar-Monster Erklärbär, Deo-Kommentar und Beipackzettel, Speculative Generality, Lazy Class, Data Class), Couplers (Feature Envy, Message Chains, Middle Man), Readability nach Clean Code (Deep Nesting, Complexity, Magic Numbers, Bad Names, Console Output, Exception Swallowing samt catch mit return, Flag Argument) und Test Smells nach Meszaros (Test-Muffel, Schauspieler). Change Preventers fehlen, sie brauchen die Historie. Rechnung ohne Deckel: jede Familie startet mit 100 Punkten, jeder Fund kostet seine Familie Punkte (nie unter 0), der Punktestand ist der Durchschnitt der Familien; ohne Tests bleibt Test Smells unbewertet. `target/clean-code-report.html` zeigt links Punktestand, Radar je Familie, Endgegner und die Funde je Familie, rechts die Dateien wie in der IDE plus eine Regelwerk-Seite mit Schwelle, Strafpunkten und Refactoring je Monster. `.clean-code-ignore` nimmt Dateien aus (Tripservice: das Original). Kein Gate. Ausschnitte mit `--nur` (Package, auch als einzelnes Segment wie `core`; Klasse, auch mit `*`; Methode als `Klasse#methode`), `--ziele` listet Packages und Klassen; ein Ausschnitt zählt nur seine Funde und schreibt den Verlauf nicht fort. In opencode übersetzt das Modell unscharfe Angaben („alle Domain-Klassen“) über `--ziele` in scharfe `--nur`-Aufrufe; die Anleitung dazu steht im Skill. Probe mit gpt-5.4-mini am Referenzautomaten (`/clean-code-report für alle Domain-Klassen`): das Modell rief `--nur` zuerst mit voll qualifizierten Klassennamen auf, was das Werkzeug damals ablehnte (seit 23.09. erlaubt), wechselte dann auf das Package `de.sharpsharp.vendingmachine.core` und meldete 93 von 100 mit korrekt benanntem Ausschnitt; die schwächste Familie nannte es falsch (angeblich alle bei 100, tatsächlich Bloaters 72), deshalb druckt das Werkzeug die schwächste Familie jetzt selbst. Version 2.1 (23.09.): Zeilen in `.clean-code-ignore` mit `?` davor blenden Dateien nur im Gesamtbericht aus, ein `--nur`-Ziel holt sie; gedacht für absichtlich schlechte Übungsbeispiele, die aus der Kata-Auswertung herausbleiben sollen (Sebastians Smell-Beispiele liegen nur lokal, nicht auf GitHub).

Geprüft am Branch `beispiel-legacy` (34 von 100, Legacy-Legende: Bloaters 4, Dispensables 0, Readability 0, Tests 30; findet die Mehrzweck-Variable `n`, das Feld `tmp` als Camper, die Absatz-Kommentare als Deo und den Beipackzettel am Feld `c`), am sauberen Startstand (100), an Gilded Rose (71, Lehrling: Readability 0 durch Pfeil, Strudel und Magie, dazu Feature Envy auf `items.get(i)`, Weichensteller über `getName()`, `Item` als Data Class), an TripService (96, ohne Tests) und an der Referenzlösung des Automaten (88, Geselle: zwei Methoden über 20 Zeilen, Zahlen 100 und 400, Schleifen und ein Verzweigen in Tests, ein Test mit zehn Prüfungen). Gegen Fehlalarme kalibriert: Records ohne Feldregeln, lange Warum-Kommentare erlaubt, Texte nur magisch bei Vergleich oder Wiederholung, Schleifenvariablen keine Temps, `main` darf drucken, Annotationen sind keine Zwillinge. `/clean-code-report` mit gpt-5.4-mini geprobt: Der Command gibt dem Modell die komplette Fundliste (`--alle`) mit; am Legacy-Beispiel antwortet es ohne Umwege mit Punkten, Rang, schwächster Familie, Endgegner `add()`, drei Monstern und Extract Method als erstem Schritt (ein Beispiel-Zeilenverweis war falsch zugeordnet). Ohne die Liste hatte es zuvor im HTML gewühlt und sich im Tripservice einen Endgegner ausgedacht, wo keiner ist; daher druckt das Werkzeug jetzt „Endgegner: keiner“.
