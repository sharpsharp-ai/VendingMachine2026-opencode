...
Clean-Code-Report (2026-09-22, in allen drei Kata-Repos als opencode-Skill `.opencode/skills/clean-code-report/`): `java .opencode/skills/clean-code-report/CleanCodeReport.java` oder in opencode `/clean-code-report`. Das Java-Programm ohne Abhängigkeiten liest `src/`, findet 14 Smell-Arten (lange Methoden, Verschachtelung, Komplexität, magische Zahlen und Texte, Zwillinge, kryptische Namen, Kommentare, toter Code, System.out, leere catch-Blöcke, Parameter, Klassenlänge, öffentliche Felder, Test-Smells) und schreibt `target/clean-code-report.html`: links Punktestand mit Rang, Endgegner-Methode und die Funde je Monster, rechts die Dateien wie in der IDE (Tabs, Klassenname, Syntaxfarben, Symbole im Gutter, Hinweise im Code, Verschachtelungstiefe als Streifen); ein Klick links springt rechts zur Zeile. Verlauf über `.clean-code-history`. Kein Gate. Der Skill-Ordner plus Command und eine Bash-Freigabe lassen sich in jedes Java-Projekt kopieren.

Geprüft am Branch `beispiel-legacy` des StringCalculators (alle neun Schritte grün, alles in `add()`: 5 von 100), am sauberen Startstand (100), an Gilded Rose (57), TripService (95, die Kopie `TripService_Original` ist der Zwilling) und an der Referenzlösung des Automaten (61); zwei Fehlalarme dabei behoben (private Konstruktoren, Tests mit vier Prüfungen). `/clean-code-report` mit gpt-5.4-mini geprobt: Punkte, Endgegner, drei Monster mit Fundstelle, erster Schritt Extract Method, keine Änderung am Code.
