...
Clean-Code-Report (2026-09-22, in allen drei Kata-Repos): `java tools/CleanCodeReport.java` liest `src/`, findet 14 Smell-Arten (lange Methoden, Verschachtelung, Komplexität, magische Zahlen und Texte, Zwillinge, kryptische Namen, Kommentare, toter Code, System.out, leere catch-Blöcke, Parameter, Klassenlänge, öffentliche Felder, Test-Smells) und schreibt `target/clean-code-report.html`: Punktestand mit Rang, Endgegner-Methode, Monster-Galerie, Karte mit Verschachtelungstiefe je Zeile, Quest-Log, Verlauf über `.clean-code-history`. Ein Java-Programm ohne Abhängigkeiten, kein Gate. Geprüft am Branch `beispiel-legacy` des StringCalculators (alle neun Schritte grün, alles in `add()`: 5 von 100), am sauberen Startstand (100), an Gilded Rose (57), TripService (95, die Kopie `TripService_Original` ist der Zwilling) und an der Referenzlösung des Automaten (61); zwei Fehlalarme dabei behoben (private Konstruktoren, Tests mit vier Prüfungen).
